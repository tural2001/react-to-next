...
class Home extends Component {
  ...
  calculateRotationAngle() {
    const { speedMbps } = this.state;
    const maxSpeed = Math.max(...speedMbps);

    // Define the speed-degree mapping points
    const speedPoints = [
      { speed: 0.2, degree: -155 },
      { speed: 1, degree: -120 },
      { speed: 5, degree: -80 },
      { speed: 10, degree: -34 },
      { speed: 20, degree: 10 },
    ...
    // Check if the elapsed time is less than 1 second (1000 milliseconds)
    const currentTime = Date.now();
    const elapsedTime = currentTime - this.startTime;

    if (elapsedTime < 1000) {
      return -155; // Stay at -155 degrees for 1 second
    }

    // Find the appropriate range for interpolation after 1 second
    for (const point of speedPoints) {
      if (point.speed <= maxSpeed) {
        lowerSpeed = point.speed;
        lowerDegree = point.degree;
      } else {
        upperSpeed = point.speed;
        upperDegree = point.degree;
        break;
      }
    }

    // Perform linear interpolation
    const percentage = (maxSpeed - lowerSpeed) / (upperSpeed - lowerSpeed);
    const rotationAngle =
      lowerDegree + percentage * (upperDegree - lowerDegree);
                  ...
                  <defs>
                    ...
                    >
                      ...
                      <feColorMatrix
                        ...
                        result="hardAlpha"
                      />
                      <feOffset dx="2.48574" dy="6.62865" />
                      <feGaussianBlur stdDeviation="3.31432" />
                      <feColorMatrix
                        type="matrix"
              ...
              <div className="d-flex flex-column align-items-center">
                ...
                <div className="flex flex-col justify-center items-center gap-10 text-white">
                  ...
                  {this.state.speedMbps.length < 6 ? (
                    ...
                    <button
                      type="button"
                      className="flex justify-center items-center speed_button"
