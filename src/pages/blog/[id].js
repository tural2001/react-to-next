...
const blog = ({ BlogData, id }) => {
  ...
  const currentPage =
    BlogData.data.findIndex((item) => item.id === parseInt(id)) + 1;
  ...
  const goToPreviousPage = async () => {
    if (currentPage > 1) {
      ...
      const previousBlogId = BlogData.data[currentPage - 2]?.id;
  ...
  const goToNextPage = async () => {
    if (currentPage < totalPages) {
      setIsNavigating(true);
      const nextBlogId = BlogData.data[currentPage]?.id;
      ...
      {paginatedBlogs.map((item) => {
        ...
        return (
          <>
            ...
            <div
              className="container max-w-[1100px] w-full  mt-6 mx-auto"
              key={item.id}
            >
            ...
            <div className="flex justify-between max-w-[1000px] mx-auto items-center my-10">
              <button
